...
export function normalizeSchedule(raw: Record<string, unknown>, planId: string): Schedule {
  ...
  return {
    ...
    address: typeof raw.address === 'string' ? raw.address.trim().slice(0, 80) : '',
    lat: Number.isFinite(raw.lat as number) ? (raw.lat as number) : null,
    lon: Number.isFinite(raw.lon as number) ? (raw.lon as number) : null,
    date,
    startTime,
